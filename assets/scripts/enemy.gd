...
func explosion_control():
	...
	if GLOBAL.shield == false:
		if GLOBAL.contadorParaShield < 2:
			...
			print("SHIELD LISTO :",GLOBAL.prepareShield)
		elif GLOBAL.contadorParaShield == 2:
...
func set_sprite(name_sp):
	$Area2D/Sprite2D.texture = load(name_sp)
